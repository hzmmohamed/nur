...
// Mock data model types (normally imported from your data model)
interface Layer {
  id: string;
  name: string;
...
// Mock VideoEditingProject class methods
interface VideoEditingProjectMethods {
  getAllLayers: () => {
    get: (key: keyof Layer) => any;
    toObject: () => Partial<Layer>;
  }[];
  addLayer: (data: Omit<Layer, "id" | "createdAt" | "updatedAt">) => string;
  ...
  return (
    <>
      <Card
        ...
        } ${layer.isLocked ? "border-amber-300" : ""}`}
      >
        <CardContent className="p-3">
          <div className="flex items-center gap-3">
            ...
            {/* Layer Type Icon */}
            <div className={`p-1.5 rounded ${typeColor} bg-opacity-20`}>
...
interface LayersPanelProps {
  project: VideoEditingProjectMethods;
  className?: string;
}
...
export default function LayersPanel({
  ...
  const [layers, setLayers] = useState<Layer[]>(() => {
    return project.getAllLayers().map((layer) => layer.toObject() as Layer);
  });
